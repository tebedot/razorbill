...
export default function Strands({
  ...
  useEffect(() => {
    ...
    let currentPalette = buildPalette(propsRef.current.colors);
    ...
    const update = (t: number) => {
      ...
      const targetPalette = buildPalette(current.colors);
      for (let i = 0; i < MAX_COLORS; i++) {
        currentPalette[i][0] += (targetPalette[i][0] - currentPalette[i][0]) * 0.05;
        currentPalette[i][1] += (targetPalette[i][1] - currentPalette[i][1]) * 0.05;
        currentPalette[i][2] += (targetPalette[i][2] - currentPalette[i][2]) * 0.05;
      }
      program.uniforms.uColors.value = currentPalette;
